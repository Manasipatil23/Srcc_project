      ...
      <div style={{ display: 'flex', gap: '1rem', borderBottom: '1px solid var(--border)', paddingBottom: '0.5rem' }}>
        {['Upcoming', 'Past', 'Cancelled'].map(tab => (
          <button 
            ...
            style={{
              padding: '0.5rem 1rem', fontWeight: 600, borderBottom: activeTab === tab ? '2px solid var(--primary)' : '2px solid transparent',
              color: activeTab === tab ? 'var(--primary)' : 'var(--text-secondary)', transition: 'all var(--transition-fast)'
            }}
          >
            {tab}
          </button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          {filteredAppointments.length > 0 ? filteredAppointments.map((apt, index) => (
            ...
            >
              {/* Timeline Dot */}
              <div style={{
                position: 'absolute', top: '24px', left: '-1.5rem', width: '14px', height: '14px',
                borderRadius: '50%', backgroundColor: 'var(--bg-surface)', border: `2px solid ${apt.status === 'Upcoming' ? 'var(--primary)' : apt.status === 'Completed' ? 'var(--success)' : 'var(--error)'}`,
                zIndex: 1
              }}></div>
